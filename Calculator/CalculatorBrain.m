...
+(NSSet *)variablesUsedInProgram:(id)program{
    NSMutableSet *_variablesUsedInProgram = [[NSMutableSet alloc] init];
    if([program isKindOfClass:[NSArray class]]){
        for (id element in program){
            if ([element isKindOfClass:[NSString class]]){
                if(![self isOperation:element]){
                    [_variablesUsedInProgram addObject:element];
                }
            }
        }
    }
    ...
    else {return _variablesUsedInProgram;}
}
    ...
    for (i=0;i<stack.count;i++){
        if ([[stack objectAtIndex:i] isKindOfClass:[NSString class]]){
            for (id key in variableValues){
                ...
                } else if([[stack objectAtIndex:i] isEqual:key]){
